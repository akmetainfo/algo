...
/*
    Time: O(n) Time complexity is O(n)O(n)O(n) because accessing the counter table is a constant time operation.
    Space: O(n) for Dictionary or O(1) when char[26] or char[256] is used.
*/
