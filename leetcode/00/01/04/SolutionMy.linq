...
public class Solution
{
    ...
    public int Path(TreeNode root, int len)
    {
        if(root == null)
            return len;
        ...
        return Math.Max(Path(root.left, len), Path(root.right, len));
    }
}
...
public class Solution2
{
    ...
    private void TraverseTree(TreeNode node, IList<int> result, int depth = 0)
    {
        if (node == null)
            return;
