...
/*
    Time: O(n) All builtin language functionality used here runs in either O(n) or O(1) time, so the entire algorithm runs in linear time.
    Space: O(n) split returns an array of O(n) length, so the algorithm uses linear additional space.
*/
